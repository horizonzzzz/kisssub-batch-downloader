...
describe("OptionsPage", () => {
  ...
  it("renders the redesigned general workspace on the default route", async () => {
    ...
    const configGroup = screen.getByTestId("options-sidebar-group-config")
    expect(within(configGroup).getByText("配置中心")).toBeInTheDocument()
    expect(within(configGroup).getAllByRole("button")).toHaveLength(4)
    expect(within(configGroup).getByRole("button", { name: "下载器与基础设置" })).toBeInTheDocument()
    expect(within(configGroup).getByRole("button", { name: "站点配置" })).toBeInTheDocument()
    expect(within(configGroup).getByRole("button", { name: "过滤规则" })).toBeInTheDocument()
    expect(within(configGroup).getByRole("button", { name: "订阅" })).toBeInTheDocument()

    const activityGroup = screen.getByTestId("options-sidebar-group-activity")
    expect(within(activityGroup).getByText("记录与命中")).toBeInTheDocument()
    expect(within(activityGroup).getAllByRole("button")).toHaveLength(2)
    expect(within(activityGroup).getByRole("button", { name: "订阅命中" })).toBeInTheDocument()
    expect(within(activityGroup).getByRole("button", { name: "批次历史" })).toBeInTheDocument()

    const overviewGroup = screen.getByTestId("options-sidebar-group-overview")
    expect(within(overviewGroup).getByText("站点总览")).toBeInTheDocument()
    expect(within(overviewGroup).getAllByRole("button")).toHaveLength(1)
    expect(within(overviewGroup).getByRole("button", { name: "源站概览" })).toBeInTheDocument()
