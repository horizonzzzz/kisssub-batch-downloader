...
export function BatchPanelLauncher({
  ...
  return (
    <ContentButton
      ...
      aria-label={i18n.t("batch.panel.expand")}
      onClick={onExpand}>
      <span className="relative inline-flex h-[24px] w-[24px] items-center justify-center">
        ...
        {selectedCount > 0 ? (
          <span
            ...
            aria-label={i18n.t("batch.panel.selectedCount", [selectedCount])}>
            {selectedCount}
          </span>
        ) : null}
      </span>
      <span className="tracking-[0.02em]">{i18n.t("batch.actions.download")}</span>
    </ContentButton>
  )
}
